...
{    
    ...
    //Importando a matricula e ID do usuario/aluno
    $matricula = $dados_discuser['matricula']; 
    $id_discente = $dados_discuser['id_discente'];

    $arquivo = $_FILES['comprovante_card_vac'];
    //Pegando somente se o arquivo for .pdf
    preg_match("/\.(pdf){1}$/i", $arquivo["name"], $ext);

    if($ext == true){
        $nome_arquivo = $matricula. '.' . $ext[1];

        $caminho_arquivo = "../../Assets/img/imagens_vacina/" . $nome_arquivo;
    ...
    $data = array(
        'carteirinha_vacinacao' =>  $nome_arquivo, 
 ...
 */
    ...
    $card_vacina['name'] = $matricula . $ext; //Definindo um novo nome 
    ...
    // Atribui dados ao POST
    $data = array('carteirinha_vacinacao' =>  $b64Doc, 'id_discente' => $id_discente);   
    ...
    $file_name =$_FILES['comprovante_card_vac'];
    //$file = new CURLFile('C:\Users\cpcri\Downloads\localDeProva.pdf', 'application/pdf', 'Myfile');
    print_r($file_name);

    $dados = [
        'carteirinha_vacinacao' => $file,
        'id_discente' => $id_discente
    ];
    ...
    curl_setopt($curl, CURLOPT_POSTFIELDS, ['carteirinha_vacinacao' => $file,
                                            'id_discente' => $id_discente]);
...
/*     //Pegando extensão do arquivo
    ...
    $card_vacina = array (
        'name' => $matricula . $ext,
        'type' => $_FILES['comprovante_card_vac']['type'],
        'tmp_name' => $_FILES['comprovante_card_vac']['tmp_name']
    );



    
    $target = "C:/Users/cpcri/Downloads/"; 
    
    $target = $target.basename($_FILES['comprovante_card_vac']['name']) ; 
    ...
    $pdf_file = new CURLFile($target,'pdf');
       
    $post_data = array(
          "carteirinha_vacinacao" => $pdf_file,
          'id_discente' => $id_discente
        );

    //transformando array em json
    $arquivo_json = json_encode($post_data, JSON_UNESCAPED_SLASHES);
    print_r($arquivo_json);   
    ...
    curl_setopt($curl, CURLOPT_POSTFIELDS, $arquivo_json);
    curl_setopt($curl, CURLOPT_HTTPHEADER, $headers);
    curl_setopt($curl, CURLOPT_CUSTOMREQUEST,'PUT');  
    $response = curl_exec($curl);
    ...
    $file_name =$_FILES['comprovante_card_vac']['name'];
 // $file_name =$_FILES['comprovante_card_vac']['tmp_name'];
    $file_ext = strtolower( end(explode('.',$file_name)));


    $file_size=$_FILES['comprovante_card_vac']['size'];
    $file_tmp= $_FILES['comprovante_card_vac']['tmp_name'];
    echo $file_tmp;
    echo "<br>";
    ...
    $comprovante_vacc = array(
        'carteirinha_vacinacao' => $b64  //base64_encode($data),
        'id_discente' => $id_discente
    );

  
    //transformando array em json
    $arquivo_json = json_encode($comprovante_vacc);
    print_r($arquivo_json);
...
// curl_close($ch);
    ...
    // $cfile = new CURLFile($card_vacina['tmp_name']['type'], $card_vacina['name']);
